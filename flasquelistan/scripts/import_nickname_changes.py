...
def run(path):
    ...
    for db in sorted(strequelistan):
        db_queries.append((db, "SELECT id, first_name, last_name, nickname "
                               "FROM EmailUser_myuser WHERE nickname != '';"))
    ...
    with models.db.session.begin():
        for path, query in db_queries:
            ...
            timestamp = datetime(int(date[:4]), int(date[4:6]), int(date[6:8]),
            ...
            for line in lines:
                ...
                if nickname != users[user_id]:
                    print(f"Between {last_timestamp.isoformat()} and {timestamp.isoformat()}, "
                          f"{first_name} {last_name} ({user_id}) got nickname '{nickname}'.")
